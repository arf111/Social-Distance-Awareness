...
def draw_RiskLine(img, minm_dist, centroids_arr, transformed_centroids, warped_imgH, warped_imgW):
...
def show_Detection(img, centroids_arr, transformed_centroids, det_points, warped_imgH, warped_imgW):
    ...
    if len(transformed_centroids) >= 2:
        for i in range(len(transformed_centroids)):
            ...
            if not ((transformed_centroids[i][0] > warped_imgW or transformed_centroids[i][0] < 0) or (transformed_centroids[i][1] > warped_imgH or transformed_centroids[i][1] < 0)):
                # Draw BBoxes, centroids and write down class_name.
